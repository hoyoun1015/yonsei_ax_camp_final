...
만들고자 한 것은 계산 하나를 자동화하는 도구가 아닙니다. 사람이 화학 가설을 문장으로
주면 **무엇과 무엇을 비교할지 스스로 정하고, 계산을 설계해 실제로 실행하고, 그 증거가
결론을 내리기에 충분한지 검토하고, 부족하면 더 정밀한 계산을 선택해, 최종적으로
지지·기각·판단보류까지 가는** AI 연구 에이전트입니다.

이 전체 과정에서 특히 어려운 과학적 판단이 하나 있습니다. **지금 얻은 계산 결과만으로
결론을 내려도 되는가, 아니면 더 정밀한 계산이 필요한가.**
...
결론을 내리기 쉽습니다. 그래서 이 연구는 두 층으로 물었습니다.

> **큰 질문 — AI 연구 에이전트가 실제 계산화학 도구를 써서 화학 가설을 검증하는
> 전 과정을 자율적으로 수행할 수 있는가?**
>
> **그 안의 핵심 설계 질문 — 지금의 계산 증거가 충분한지를 "이 계산법은 원래 어느
> 정도 틀릴 수 있는가"로 판단하게 하면, 에이전트의 과학적 의사결정이 달라지는가?**
...
연구 설계와 동결의 근거는 [docs/기획안_v3.md](docs/기획안_v3.md)와
[docs/DECISION_LOG.md](docs/DECISION_LOG.md), 그리고 동결 산출물(`data/tasks/frozen_*.json`)에
있고, 재현 절차는 [docs/REPRODUCIBILITY.md](docs/REPRODUCIBILITY.md)에 있습니다.
([CLAUDE.md](CLAUDE.md)는 개발·운영 지침이며 연구 프로토콜 문서가 아닙니다.)
